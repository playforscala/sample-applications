...
object Products extends Controller {
  ...
  def newProduct = Action { implicit request =>
    val form = if (flash.get("error").isDefined) {
      val errorForm = this.newProductForm.bind(flash.data)
  ...
  def save = Action { implicit request =>
    val newProductForm = this.newProductForm.bindFromRequest()
  ...
  def edit(ean: Long) = Action { implicit request =>
    val form = if (flash.get("error").isDefined)
      this.updateProductForm(ean).bind(flash.data)
    else
      this.updateProductForm(ean).fill(Product.findByEan(ean).get)
  ...
  def update(ean: Long) = Action { implicit request =>
    if (Product.findByEan(ean).isEmpty)
      ...
      val updatedProductForm = this.updateProductForm(ean).bindFromRequest()
      ...
      updatedProductForm.fold(
        ...
        success = { updatedProduct =>
          val product = Product.findByEan(ean).get.copy(
            updatedProduct.ean,
            updatedProduct.name,
          ...
          Redirect(routes.Products.show(product.ean)).flashing(successMessage)
        }
      )
    }
  }
